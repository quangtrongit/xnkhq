...
<form name="supplier" id="validate" class="form" action="index.php?com=slider&act=save_photo&id_slider=<?=$_REQUEST['id_slider']?>&type=<?=$_REQUEST['type']?><?php if($_REQUEST['p']!='') echo'&p='.$_REQUEST['p'];?>" method="post" enctype="multipart/form-data">
    ...
    <div class="formRow <?=(!$config_s['thoigian'])?'none':''?>">   
          ...
          <div class="formRight">
              <input type="date" name="thoigian<?=$i?>" title="Nhập thời gian" id="thoigian" class="tipS validate[required]" value="<?=Date('Y-m-d',time())?>" />
          </div>
          <div class="clear"></div>
        </div>    
        ...
        <div id="content_lang_<?=$key?>" class="tab_content">     
         <?php for($i=0; $i<4; $i++){?>
            ...
            <label><?php if($_GET['type']=='y-kien-khach-hang'){
                echo 'Tên khách hàng';}else{ echo 'Tên hình ảnh'; }?> <?=$i+1?>:</label>
            <div class="formRight">
            ...
            <div class="formRow <?=(!$config_s['mota'])?'none':''?>">   
                ...
                <div class="formRight">
                    <textarea rows="8" type="text" name="mota<?=$key?><?=$i?>" title="Nhập tên hình ảnh <?=$i+1?>" id="mota<?=$key?><?=$i?>" class="tipS" value="" /><?=@$item['mota'.$key]?></textarea>
                </div>
                <div class="clear"></div>
            </div>
        <?php }?>
