...
<form name="supplier" id="validate" class="form" action="index.php?com=product&act=save_danhmuc<?php if($_REQUEST['p']!='') echo'&p='.$_REQUEST['p'];?><?php if($_REQUEST['type']!='') echo'&type='. $_REQUEST['type'];?>" method="post" enctype="multipart/form-data">
   ...
   <div id="info" class="tab_content">
       <div class="formRow <?= (!$config_s['logo'])?'none':'' ?>">
    			<label>Tải logo:</label>
    			<div class="formRight">
                	<input type="file" id="file1" name="file1" />
    				<img src="./images/question-button.png" alt="Upload hình" class="icon_question tipS" original-title="Tải hình ảnh (ảnh JPEG, GIF , JPG , PNG)">
    				<div class="note"> <?=$config_s['size_logo']?>  <?=_format_duoihinh_l?> </div>
    			</div>
    			<div class="clear"></div>
    		</div>
     <?php if($_GET['act']=='edit_danhmuc'){?>
    		<div class="formRow <?= (!$config_s['logo'])?'none':'' ?>">
    			<label>Hình Logo Hiện Tại :</label>
    			<div class="formRight">
    			
    			<div class="mt10"><img src="<?=_upload_sanpham.$item['logo']?>"  width="100px" alt="NO PHOTO" width="100" /></div>
    			</div>
    			<div class="clear"></div>
    		</div>
	    <?php } ?>
   ...
   <?php if($_GET['act']=='edit_danhmuc'){?>
		<div class="formRow <?= (!$config_s['hinhanh'])?'none':'' ?>">
			...
			<div class="mt10"><img src="<?=_upload_sanpham.$item['photo']?>"   alt="NO PHOTO" width="400px" /></div>
    ...
    <div class="formRow ">
     ...
     <div class="formRow  none">
          <label>Nổi bật footer: <img src="./images/question-button.png" alt="Chọn loại" class="icon_que tipS" original-title="Bỏ chọn để không hiển thị danh mục này ! "> </label>

         <div class="formRight">
            <input type="checkbox" name="noibat_tc" id="check1" <?=(!isset($item['noibat_tc']) || $item['noibat_tc']==1)?'checked="checked"':''?> />
            </div>
			   <div class="clear"></div>
      </div>
      <div class="formRow">
