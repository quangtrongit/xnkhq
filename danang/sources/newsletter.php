...
function get_mail(){
	...
	if(!$id)
		transfer("Không nhận được dữ liệu", "index.php?com=newsletter&act=man".$urlcu);
	
	$sql = "select * from #_newsletter where id='".$id."'";
	$d->query($sql);
	if($d->num_rows()==0) transfer("Dữ liệu không có thực", "index.php?com=newsletter&act=man".$urlcu);
	$item = $d->fetch_array();
}
...
function save_mail(){
	...
	if(empty($_POST)) transfer("Không nhận được dữ liệu", "index.php?com=newsletter&act=man".$urlcu);
	...
	if($id){
		...
		if($d->update($data))
			...
			transfer("Cập nhật dữ liệu bị lỗi", "index.php?com=newsletter&act=man".$urlcu);
	}else{
		$data['type'] = $_POST['type'];
		$data['name'] = $_POST['name'];
		$data['phone'] = $_POST['phone'];	
		...
		if($d->insert($data))
			...
			transfer("Lưu dữ liệu bị lỗi", "index.php?com=newsletter&act=man".$urlcu);
	}
}
...
function delete(){
	...
	if(isset($_GET['id'])){
		...
		if($d->query($sql))
			...
			transfer("Xóa dữ liệu bị lỗi", "index.php?com=newsletter&act=man".$urlcu);
	}
...
function send(){
	...
	if(isset($_POST['listid'])){
		...
		$mail->SetFrom($mail_host, $company_mail['ten']);
		$mail->AddAddress($company_mail['email'], $company_mail['ten']);
		...
		for ($i=0 ; $i<count($listid) ; $i++){
			...
			$id =  themdau($idTin);		
			$d->reset();
			$sql = "select email,name from #_newsletter where id='".$id."'";
			$d->query($sql);
			if($d->num_rows()>0){
